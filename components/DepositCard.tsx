...
export default function DepositCard({ type, label, tag, shares, count }: Props) {
  const t = THEME[type];
  return (
    <div className={`rounded bg-paper border-[1.3px] ${t.border} px-5 pt-[22px] pb-5 mb-4`}>
      <span className={`block text-[10.5px] tracking-[0.2em] uppercase font-bold mb-2.5 ${t.tagText}`}>
        {tag}
      </span>
      <h3 className={`font-serif font-bold text-[21px] mb-[18px] tracking-wide ${t.titleText}`}>
        {label}
      </h3>
      <div className="flex gap-[26px] mb-[18px]">
        <div>
          ...
          <span className="text-[10.5px] uppercase tracking-wide text-ink-faint">Total Shares</span>
        </div>
        <div>
          <span className="block text-[25px] font-bold text-ink tabular-nums">{count}</span>
